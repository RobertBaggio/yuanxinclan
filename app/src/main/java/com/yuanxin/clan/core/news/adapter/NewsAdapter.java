...
public class NewsAdapter extends BaseMultiItemQuickAdapter<NewEntity, BaseViewHolder> {
    public NewsAdapter(@Nullable List<NewEntity> data) {
        super(data);
...
//                helper.setImageUrl(R.id.iv, item.getContent());
                ImageManager.loadRoundCornerImageWithMargin(mContext, item.getImg(), R.drawable.list_img, (ImageView) helper.getView(R.id.item_gank_image), 0);
...
//        helper.setText(R.id.item_gank_who, TextUtil.isEmpty(item.getEpShortNm())?item.getEpNm():item.getEpShortNm());//类型
                ...
                if (item.getCreateDt() == null) {
                    ...
                    try{
                        ...
                        helper.setText(R.id.item_gank_createdAt, DateDistance.getDistanceTime(item.getCreateDt(),thistime));
                    }catch(Exception e){
                        e.printStackTrace();
                    }
