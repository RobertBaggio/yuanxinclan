...
import static com.yuanxin.clan.core.huanxin.InviteMessage.InviteMesageStatus.BEINVITEED;
...
public class DemoDBManager {
     ...
     * @param usernameId
     */
    synchronized public EaseUser getContact(String usernameId) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        EaseUser user = null;
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + UserDao.TABLE_NAME + " where " + UserDao.COLUMN_NAME_ID + " = " + usernameId, null);
            while (cursor.moveToNext()) {
                String username = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_ID));
                String nick = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_NICK));
                String avatar = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_AVATAR));
    ...
    synchronized public Map<String, RobotUser> getRobotList() {
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                String username = cursor.getString(cursor.getColumnIndex(UserDao.ROBOT_COLUMN_NAME_ID));//id
                String nick = cursor.getString(cursor.getColumnIndex(UserDao.ROBOT_COLUMN_NAME_NICK));//昵称
                String avatar = cursor.getString(cursor.getColumnIndex(UserDao.ROBOT_COLUMN_NAME_AVATAR));//头像
