...
public class NearAndYearActivity extends BaseActivity implements PullToRefreshView.OnFooterRefreshListener,PullToRefreshView.OnHeaderRefreshListener{
    ...
    protected void initView(Bundle savedInstanceState, Intent intent) {
        ...
        if (type.equals("n")){
            mTextView.setText("附近活动");
        }else {
            mTextView.setText("活动预报");
        }
    ...
    private void initRecyclerView() {
        mMyTradesStatusAdapter = new NeardtradesAdapter(NearAndYearActivity.this, mEntities);
        mMyTradesStatusAdapter.setOnItemClickListener(new NeardtradesAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                Intent intent = new Intent(NearAndYearActivity.this, TradesDetailActivity.class);
    ...
    private void getBusinesslist(int id) {
        ...
        if (type.equals("n")){
            url = Url.getnearPage;
            params.put("lon", MyShareUtil.getSharedString("lot"));
            params.put("lat", MyShareUtil.getSharedString("lat"));
        }else {
            url = Url.newmarketlist;
            final Calendar c = Calendar.getInstance();
            c.setTimeZone(TimeZone.getTimeZone("GMT+8:00"));
            String year = String.valueOf(c.get(Calendar.YEAR));// 获取当前月份的日期号码
