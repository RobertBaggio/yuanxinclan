 ...
 * Describe:
 * Author: xjc
 * Date: 2017/11/9 0009 15:51
 * Copyright (c) 2017, tonixtech.com All Rights Reserved.
 */

public class OutputXuQiuActivity extends BaseActivity {
    ...
    ImageView gyimage6;
    @BindView(R.id.xqtitleet)
    EditText xqtitleet;
    @BindView(R.id.xqnumet)
    EditText xqnumet;
    @BindView(R.id.xqmoneyet)
    EditText xqmoneyet;
    @BindView(R.id.xqzhouqistartet)
    EditText xqzhouqistartet;
    @BindView(R.id.xqzhouqiendet)
    TextView xqzhouqiendet;
    @BindView(R.id.xqhyte)
    TextView xqhyte;
    @BindView(R.id.xqyouxiaoqite)
    TextView xqyouxiaoqite;
    @BindView(R.id.xqjiaofudiqute)
    TextView xqjiaofudiqute;
    @BindView(R.id.activity_yuan_xin_crowd_middle_text)
    TextView activity_yuan_xin_crowd_middle_text;
    @BindView(R.id.xqneironget)
    EditText xqneironget;
    @BindView(R.id.xqdqli)
    LinearLayout xqdqli;
    @BindView(R.id.xqhyli)
    LinearLayout xqhyli;
    @BindView(R.id.outputxq_button)
    Button outputxq_button;
...
//    TextView activity_yuan_xin_crowd_middle_text;
    ...
    protected void initView(Bundle savedInstanceState, Intent intent) {
        ...
        if (type.equals("my")){
            activity_yuan_xin_crowd_middle_text.setText("编辑需求");
            outputxq_button.setText("确认发布");
        ...
        fileUploadOnNextListener = new SubscriberOnNextListener<BaseJsonEntity>() {
            ...
            public void onNext(BaseJsonEntity t) {
                if (t.getSuccess().equals("true")) {
                    ...
                    ToastUtil.showInfo(getApplicationContext(), t.getMsg(), Toast.LENGTH_SHORT);
                }
            }
        };
    }
    ...
    private void showdata(){
        xqtitleet.setText(mGongXuEntity.getTitle());
        xqnumet.setText(mGongXuEntity.getNumber());
        xqmoneyet.setText(mGongXuEntity.getBudget());
        xqzhouqistartet.setText(mGongXuEntity.getDeliveryCycle());
        xqyouxiaoqite.setText(mGongXuEntity.getEndTime());
        xqjiaofudiqute.setText(mGongXuEntity.getAddress());
        xqhyte.setText(mGongXuEntity.getIndustryNm());
        xqneironget.setText(mGongXuEntity.getContent());
    ...
    @OnClick({R.id.rlLeft,R.id.gyimage1,R.id.gyimage2,R.id.gyimage3,R.id.gyimage4,R.id.gyimage5,R.id.gyimage6,R.id.outputxq_button,
            R.id.xqhyli,R.id.xqdqli,R.id.xqriqili})
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.xqriqili:
                ...
                break;
            case R.id.xqhyli:
                Intent publishIntent = new Intent(OutputXuQiuActivity.this, BusinessDistrictClassifyActviity.class);
                startActivityForResult(publishIntent, REQUEST_INDUSTRY);
                break;
        }
    }
    public boolean ifokOutput(){
        if (TextUtil.isEmpty(xqtitleet.getText().toString())){
            ToastUtil.showInfo(this, "请输入所需具体物资", Toast.LENGTH_SHORT);
            return false;
        }
        if (TextUtil.isEmpty(xqnumet.getText().toString())){
            ToastUtil.showInfo(this, "请输入数量", Toast.LENGTH_SHORT);
            return false;
        }
        if (TextUtil.isEmpty(xqyouxiaoqite.getText().toString())){
            ToastUtil.showInfo(this, "请完善有效日期", Toast.LENGTH_SHORT);
            return false;
        }
//        if (TextUtil.isEmpty(xqzhouqistartet.getText().toString())){
//            Toast.makeText(this, "请完善交付周期", Toast.LENGTH_SHORT).show();
//            return false;
//        }
        if (TextUtil.isEmpty(xqjiaofudiqute.getText().toString())){
            ToastUtil.showInfo(this, "请完善交付地区", Toast.LENGTH_SHORT);
            return false;
        }
        if (TextUtil.isEmpty(xqhyte.getText().toString())){
            ToastUtil.showInfo(this, "请选择行业", Toast.LENGTH_SHORT);
            return false;
        }
        if (TextUtil.isEmpty(xqneironget.getText().toString())){
            ToastUtil.showInfo(this, "请输入所需具体物资详情", Toast.LENGTH_SHORT);
            return false;
        }
...
//                image1 = imageUrl;
    ...
    private void addInfo() {
        ...
        String title = xqtitleet.getText().toString().trim();//名称
        String number = xqnumet.getText().toString().trim();//简介
        String money = xqmoneyet.getText().toString().trim();
        if (TextUtil.isEmpty(xqmoneyet.getText().toString().trim())){
            money="议价";
        }
        String numbertime = xqzhouqistartet.getText().toString().trim();
        if (TextUtil.isEmpty(xqzhouqistartet.getText().toString().trim())){
            numbertime="待定";
        }
        String riqitime = xqyouxiaoqite.getText().toString().trim();
        String diqu = xqjiaofudiqute.getText().toString().trim();
        String nr = xqneironget.getText().toString().trim();
        ...
        params.put("supplyDemand", 1);//需求id
    ...
    private void birthSetting() {
        ...
        new DatePickerDialog(this, new DatePickerDialog.OnDateSetListener() {
            ...
            public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
                ...
                xqyouxiaoqite.setText(new StringBuilder().append(year).append("-").append(month).append("-").append(dayOfMonth));//tvBirth就是选择日期结果textview
            }
