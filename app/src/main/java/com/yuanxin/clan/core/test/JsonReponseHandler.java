package com.yuanxin.clan.core.test;
...
public class JsonReponseHandler {
    ...
    public static <T> Object getBeanWithEntity(JSONObject response, Class<T> cls) {
        ...
        try {
            ...
            obj = JSON.parseObject(jo.toString(), cls);
        } catch (JSONException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static <T> List<T> getListFromJsonWithPageEntity(Object response,
                                                            Class<T> clazz, FatherAdapter<T> baseAdapter, Dialog dialog) {
        return getListFromJsonSingle(response, clazz,
    ...
    public static <T> List<T> getListFromJsonWithEntity(Object response,
                                                        String listKey, Class<T> clazz, FatherAdapter<T> baseAdapter,
                                                        Dialog dialog) {
        return getListFromJsonDouble(response, clazz,
     ...
     * 直接listKey
     */
    public static <T> List<T> getListFromJsonSingle(Object response, Class<T> clazz,
                                                    String listKey, FatherAdapter<T> baseAdapter, Dialog dialog) {
        return getListFromJson(response, clazz, null, listKey, baseAdapter,
                dialog);
    }
     ...
     */
    public static <T> List<T> getListFromJsonDouble(Object response, Class<T> clazz,
                                                    String beforeListKey, String listKey, FatherAdapter<T> baseAdapter,
                                                    Dialog dialog) {
        return getListFromJson(response, clazz, beforeListKey, listKey,
                baseAdapter, dialog);
    }
     ...
     * @param clazz       List的数据类型，如List<Comment>
     * @param baseAdapter 更新ListView的适配器
     * @param dialog      进度对话框
     * @author 陈福荣, 吴格非
     */
    public static <T> List<T> getListFromJson(Object response, Class<T> clazz,
                                              String beforeListKey, String listKey, FatherAdapter<T> baseAdapter,
                                              Dialog dialog) {
        if (response == null) {
            return null;
