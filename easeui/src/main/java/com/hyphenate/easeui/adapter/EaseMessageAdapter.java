...
public class EaseMessageAdapter extends BaseAdapter{
	...
	private static final int MESSAGE_TYPE_SENT_CARD_TIP = 16;
	private static final int MESSAGE_TYPE_RECV_CARD_TIP = 17;
	...
	Handler handler = new Handler() {
		private void refreshList() {
			...
			java.util.List<EMMessage> var = removeLastSameMsg(conversation.getAllMessages());
			messages = var.toArray(new EMMessage[var.size()]);
			conversation.markAllMessagesAsRead();
			notifyDataSetChanged();
		}

		private List<EMMessage> removeLastSameMsg(List<EMMessage> messages) {
			//使用LinkedHashSet删除ArrayList中重复的元素，保持插入顺序
			LinkedHashSet<EMMessage> set = new LinkedHashSet<EMMessage>(messages);
			return new ArrayList<EMMessage>(set);
		}
	...
	public int getViewTypeCount() {
	    if(customRowProvider != null && customRowProvider.getCustomChatRowTypeCount() > 0){
	        return customRowProvider.getCustomChatRowTypeCount() + 20;
	    }
        return 20;
    }
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
		    ...
		    }else if(message.getBooleanAttribute(EaseConstant.CARD_TYPE, false)) {
				return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_CARD : MESSAGE_TYPE_SENT_CARD;
			}else if(message.getBooleanAttribute(EaseConstant.CARD_TIP_TYPE, false)) {
				return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_CARD_TIP : MESSAGE_TYPE_SENT_CARD_TIP;
	...
	protected EaseChatRow createChatRow(Context context, EMMessage message, int position) {
        ...
        case TXT:
            if(message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)){
				...
				chatRow = new EaseBusinessCardChatrow(context, message, position, this);
			}else if (message.getBooleanAttribute(EaseConstant.CARD_TIP_TYPE, false)) {
				chatRow = new EaseBusinessCardTipChatrow(context, message, position, this);
			}else if (message.getBooleanAttribute(ShareTypes.SHARE_URL_MSG, false)) {
				// 资讯分享
				chatRow = new EaseShareQiliaoChatrow(context, message, position, this);
			}else{
