...
//            Log.v("lgq","shouying==========="+len);
            RelativeLayout.LayoutParams linearParams =(RelativeLayout.LayoutParams) bubbleLayout.getLayoutParams(); //取控件textView当前的布局参数
            if (len<=2){
                linearParams.width = 150*1;// 控件的宽强制设成30
            }else if (len>2){
                if (len>9){
                    len=9;
                }
                linearParams.width = 50*len;// 控件的宽强制设成30
        ...
        if (EaseChatRowVoicePlayClickListener.playMsgId != null
            ...
            if (message.direct() == EMMessage.Direct.RECEIVE) {
                voiceImageView.setImageResource(R.drawable.chat_music);
            } else {
                voiceImageView.setImageResource(R.drawable.chat_my_3);
            }
        }
